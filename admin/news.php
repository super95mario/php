<?php 
	...
	if (isset($_POST['_action_']) && $_POST['_action_'] == 'add_news') {
		...
		if($_SESSION['user']['roles'] =='U')
		{$arc ='Y';}
	else
		{$arc = $_POST['archive'];}
		$query  = "INSERT INTO news (title, description, archive)";
		$query .= " VALUES ('" . htmlspecialchars($_POST['title'], ENT_QUOTES) . "', '" . htmlspecialchars($_POST['description'], ENT_QUOTES) . "', '" . $arc . "')";
	    ...
	    move_uploaded_file($temp,"news/".$_picture);
	    $query  = "INSERT INTO pictures (name) VALUES ('".$_picture."')";
		$result = @mysqli_query($MySQL, $query);
		$query  = "select id from pictures where name ='".$_picture."'";
		$result2 = @mysqli_query($MySQL, $query);
		$row = @mysqli_fetch_array($result2);

		$query  = "INSERT INTO galery (News_id,Pictures_id)values('".$ID."','".$row['id']."')";
	...
	# Update news
	if (isset($_POST['_action_']) && $_POST['_action_'] == 'edit_news'&& $_SESSION['user']['roles'] !='U' ) {
        ...
        if($_FILES['picture']['error'] == UPLOAD_ERR_OK && $_FILES['picture']['name'] != "") {
		    ...
		    move_uploaded_file($temp,"news/".$name);
		}
        }
	...
	# Delete news
	if (isset($_GET['delete']) && $_GET['delete'] != ''&& $_SESSION['user']['roles'] =='A') {
	...
	#Edit news
	else if (isset($_GET['edit']) && $_GET['edit'] != ''&& $_SESSION['user']['roles'] !='U') {
		$query  = "SELECT * FROM news";
